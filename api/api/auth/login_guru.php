...
$stmt = $conn->prepare("SELECT nipy, nama, ket, email, password FROM guru WHERE email = ?");
if ($stmt === false) {
...
if ($result->num_rows === 0) {
    ...
    $password_from_db = $guru['password'];
    ...
    if (str_starts_with($password_from_db, '$2y$') || str_starts_with($password_from_db, '$2a$') || str_starts_with($password_from_db, '$argon2id$')) {
        ...
        if ($is_password_valid) {
            ...
            // Update password di tabel 'guru' menggunakan 'nipy'
            $stmt_update = $conn->prepare("UPDATE guru SET password = ? WHERE nipy = ?");
            if ($stmt_update) {
                $stmt_update->bind_param("ss", $hashed_password, $guru['nipy']);
                $stmt_update->execute();
                $stmt_update->close();
                error_log("Password guru NIPY {$guru['nipy']} berhasil di-hash saat login.");
            } else {
                error_log("Gagal menyiapkan update password hash untuk nis NIS {$guru['nipy']}: " . $conn->error);
            }
        }
    }
