...
// CatatanScreen adalah StatefulWidget untuk halaman catatan harian.
// Menerima parameter `nis` (Nomor Induk Siswa).
class CatatanScreen extends StatefulWidget {
  final String nis;
  const CatatanScreen({super.key, required this.nis});
...
class _CatatanScreenState extends State<CatatanScreen> {
  ...
  Future<void> _fetchCatatanAndSetState(DateTime date) async {
    ...
    try {
      ...
      print('DEBUG: Memulai panggilan API getCatatan untuk NIS ${widget.nis} dan tanggal $formattedDate');
      final catatan = await _apiService.getCatatan(widget.nis, formattedDate);
  ...
  Future<void> _fetchCatatanStatistics() async {
    if (!mounted) return;
    try {
      // Perbaikan: Ganti data dummy dengan panggilan API yang sebenarnya.
      print('DEBUG: Memuat statistik catatan...');
      final Map<String, dynamic> stats = await _apiService.getCatatanStatistics(widget.nis);
      if (mounted) {
        setState(() {
  ...
  Future<void> _addOrUpdateCatatan() async {
    print('DEBUG: _addOrUpdateCatatan() dipanggil.');
    ...
    try {
      ...
      final Catatan newCatatan = Catatan(
        nis: widget.nis,
        tanggalCatatan: formattedDate,
        catatanText: _catatanController.text.trim(),
      );
      
      print('DEBUG: Memulai panggilan API addOrUpdateCatatan.');
      final response = await _apiService.addOrUpdateCatatan(newCatatan);
      if (mounted) {
        setState(() {
          _isLoading = false;
        });
  ...
  Future<void> _deleteCatatan() async {
    ...
    try {
      final String formattedDate = DateFormat('yyyy-MM-dd').format(_selectedDate);
      print('DEBUG: Memulai panggilan API deleteCatatan untuk tanggal $formattedDate');
      final response = await _apiService.deleteCatatan(widget.nis, formattedDate);
      if (mounted) {
        setState(() {
          _isLoading = false;
        });
                ...
                children: [
                  Expanded(
                    child: ElevatedButton.icon(
                      onPressed: _isLoading ? null : _addOrUpdateCatatan,
